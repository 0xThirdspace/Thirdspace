    ...
    <div className="hidden md:block lg:block xl:block 2xl:block font-bold absolute top-0 left-0 w-full bg-opacity-75 p-4 ">
      <div className="flex justify-around font-DMSans pt-10 px-4">
        ...
        <div className="flex font-DMSans md:text-">
          {NavItems.map((item, index) => (
            <div className="" key={index}>
              <Link
                ...
                href={item.link}
              >
                <p className="md:mx-2 lg:mx-6 mt-5 md:text-xs lg:text-md xl:text-lg text-white hover:text-purple-500">
                  {item.title}
                </p>
              </Link>
            </div>
          ...
          <Image
            className="md:w-6 md:mt-4  xl:mt-4 lg:mt-2 lg:w-8 lg:mx-4 lg:mb-2"
            src={modeIcon}
            width="36"
            height="36"
            alt="mode icon"
