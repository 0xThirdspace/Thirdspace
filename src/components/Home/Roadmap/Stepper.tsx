      ...
      {roadMapItems.map((item, index) => (
        <div className="m-2" key={index}>
          <div className="flex justify-start items-center">
            ...
            <hr className="w-[225px] bg-[#2F2F2F] h-1 border-t-2 border-[#2F2F2F]" />
            <div className="rounded-full w-4 h-4 bg-[#2F2F2F]"></div>
          </div>
          <div className="text-white mt-2 ">
            <h3 className="text-3xl mt-8 font-extrabold bg-gradient-to-r from-purple-500 to-orange-500 text-transparent bg-clip-text ">
              {item.title}
            </h3>
            <p className="mt-2">{item.description}</p>
            <p className="mt-2">{item.description2}</p>
          </div>
        </div>
      ))}
    </div>
